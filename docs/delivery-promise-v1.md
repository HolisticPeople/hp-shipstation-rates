...
## Google Submit Data report v1

`hp_ss_get_google_submit_data_v1(): array` is a server-only read-only report for the GMC-owned Google Submit Data presentation. It returns `schema_version`, `version`, `generated_at`, `status`, non-sensitive `errors`, provider version, sanitized actual transit-rule configuration, handling configuration, derived country/state/service scope, and explicit matcher limitations. It never returns credentials, customer/order data, rule source text, quotes, prices or a Google submission/receipt state; it does not write options or perform network work. A malformed, absent, empty, or partly rejected rule configuration reports `unavailable` rather than ready. The current matcher does not enforce package, origin, postcode, or disruption exceptions.
